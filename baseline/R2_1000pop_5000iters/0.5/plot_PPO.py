...
sac_data = pd.read_csv('results_PPO.csv')
...
for _, row in sac_data.iterrows():
    plt.scatter(row['objective1'], row['objective2'],s=70, color=cmap(norm(row['alpha'])), label=f"PPO alpha={row['alpha']}")

# Since the labels will be redundant, we create a custom legend
from matplotlib.lines import Line2D
legend_elements = [Line2D([0], [0], marker='o', color='w', label=rf'$\alpha$ = {alpha}', markerfacecolor=cmap(norm(alpha))) for alpha in sac_data['alpha'].unique()] + [Line2D([0], [0], marker='x', color='k', label='MOPSO')]
...
plt.title('Comparison of PPO and MOPSO')
plt.grid(True)
#plt.show()
plt.savefig("PPO_vs_MOPSO.png")